...
std::vector<glm::vec2> marching_squares(
    ...
    float threshold)
{
    assert(alpha.size() == (size_t)width * height);

    struct Seg { glm::vec2 a, b; };
    std::vector<Seg> segs;
    segs.reserve((width + height) * 2);

    auto v = [&](uint32_t x, uint32_t y) { return alpha[y * width + x]; };
...
std::vector<glm::vec2> generate_shape(
    ...
    float epsilon)
{
    std::vector<glm::vec2> ms = marching_squares(width, height, alpha, threshold);
    if (ms.size() == 0) return {};
